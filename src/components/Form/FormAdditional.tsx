...
export default function FormAdditional({ open, onClose, temp }: Props) {
  ...
  const createData = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await FirebaseService.addDocument("additionals", newDoc);
    } catch (err) {
      console.log(err);
    } finally {
      onClose();
  ...
  const updateData = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await FirebaseService.updateDocument("additionals", temp?.id, newDoc);
    } catch (err) {
      console.log(err);
    } finally {
      onClose();
  ...
  return (
    ...
    >
      <form onSubmit={isAddMode ? createData : updateData}>
        <Drawer.Header>
          <Drawer.Title>
            {isAddMode ? "Cadastrar Borda Recheada" : "Editar Borda Recheada"}
          </Drawer.Title>
          <Drawer.Actions>
